...
@media(max-width:520px){.asset-masonry{--board-default-gap:calc(var(--space)/2);width:calc(100% + var(--space));margin-inline:calc(var(--space)/-2);grid-template-columns:repeat(2,minmax(0,1fr));column-gap:calc(var(--space)/2)}.asset-masonry,.asset-masonry.is-masonry{row-gap:calc(var(--space)/3)}.asset-masonry.custom-view,.asset-masonry.custom-view.is-masonry{row-gap:0}.asset-card{padding-bottom:0}.preview{border-radius:var(--radius-mobile);clip-path:inset(0 round var(--radius-mobile))}.asset-masonry.stable-columns .asset-card:nth-child(n){grid-column:auto}.asset-masonry.stable-columns .asset-card:nth-child(odd){grid-column:1}.asset-masonry.stable-columns .asset-card:nth-child(even){grid-column:2}.card-body{padding-top:calc(var(--space)/3);padding-left:calc(var(--space)/4);font-size:var(--font-size-body-compact)}}
...
@media(prefers-reduced-motion:reduce){.preview :is(img,video){transition:none}.figma-button{transition-duration:.01ms;transform:translate(-50%,0)}.preview-actions{transition:none}.figma-button:active{scale:1}}
</style>
