...
const handlePreviewClick = (event: MouseEvent) => {
  ...
  if (!canZoom.value || !gestureStartedOnPreview || gestureMoved) {
    lastTapAt = 0
    return
  }
...
const startGesture = (event: PointerEvent) => {
  ...
  allowsOpeningViewTransition.value = false
  clearTimeout(openingViewTransitionTimer)
  gestureStartedOnPreview = canZoom.value
  gestureMoved = false
  if (canZoom.value) {
...
const moveGesture = (event: PointerEvent) => {
  ...
  if (gestureAxis.value === 'pan') {
    if (Math.hypot(event.clientX - panStartX, event.clientY - panStartY) > 8) {
      gestureMoved = true
      lastTapAt = 0
    }
