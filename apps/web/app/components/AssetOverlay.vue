...
watch(asset, value => { if (value) { title.value = value.title; description.value = value.description ?? '' } }, { immediate: true })
...
ref="dialog" class="asset-dialog" :class="{ 'is-closing': isClosing }"
      ...
      <div class="overlay-toolbar"><span>Asset details</span><span v-if="asset" class="muted">Version {{ asset.version
          }}</span><button
class="close-button" type="button" aria-label="Close asset details" autofocus
      ...
      <main v-else-if="asset" class="overlay-content">
        ...
        <aside>
          ...
          <template v-if="editing"><label>Title<input
v-model="title"
                maxlength="200"></label><label>Description<textarea
...
class="button-secondary"
          ...
          <template v-else>
            <h1 id="asset-overlay-title">{{ asset.title }}</h1>
...
v-if="asset.status !== 'archived'"
              class="text-button" @click="patchAsset({ status: 'archived' })">Archive</button><button
v-if="role === 'admin'"
