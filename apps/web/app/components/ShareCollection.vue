...
:id="`board-feedback-${collection.id}`" class="field-message" :class="{ error: boardFeedback[collection.id]?.error }" role="status" aria-live="polite">{{ boardFeedback[collection.id]?.text }}</span></label><template v-else><strong>{{ collection.title }}</strong></template><span class="board-meta">{{ collection.role }}<template v-if="collection.purpose === 'review'"> · submissions on</template><template v-else-if="collection.purpose === 'portfolio'"> · portfolio</template><template v-else-if="collection.mode === 'dynamic'"> · smart board</template> · {{ collection.publication_enabled ? 'published' : 'public link off' }}<template v-if="collection.expires_at"> · expires {{ new Date(collection.expires_at).toLocaleDateString() }}</template></span></div>
  ...
  <SelectionPanel :visible="createPanelOpen" :label="createPanelTitle" wide overlay
    ...
    <AssetFilterControls
      ...
      <template #before>
        ...
        >
          <h2 class="filter-overlay-title" id="create-board-submissions">Collect submissions</h2>
