...
interface PublicAsset { id: string; title: string; description: string|null; previewUrl: string; preview2xUrl?: string|null; originalUrl?: string|null; mime_type?: string|null; width: number; height: number; projects: { name: string } | null; asset_tags: Array<{ tags: { name: string } | null }> }
interface PortfolioCase { id:string; title:string; description:string|null; layout:BoardLayout; viewSettings:BoardViewSettings; assets:PublicAsset[] }
...
const { data, status, error } = await useFetch<PublicResponse>(() => `/api/public/collections/${String(route.params.slug)}${preview.value ? '?preview=true' : ''}`)
...
<template>
  <main class="public-library">
    <p v-if="preview" class="preview-notice" role="status">Private preview · Only workspace members can view this page</p>
    ...
    <section v-else-if="collection?.purpose === 'portfolio'" class="portfolio-cases" aria-label="Portfolio boards"><article v-for="portfolioCase in cases" :key="portfolioCase.id"><div class="portfolio-case-heading"><h2>{{ portfolioCase.title }}</h2><p v-if="portfolioCase.description">{{ portfolioCase.description }}</p></div><BoardLayoutRenderer :assets="portfolioCase.assets" :layout="portfolioCase.layout" :view-settings="portfolioCase.viewSettings" :label="`${portfolioCase.title} board`"><template #details="{asset}"><p v-if="asset.description">{{ asset.description }}</p></template></BoardLayoutRenderer></article></section>
    <div v-else-if="assets.length === 0" class="state"><strong>No liked items yet</strong><span>This collection will show items when they are available.</span></div>
