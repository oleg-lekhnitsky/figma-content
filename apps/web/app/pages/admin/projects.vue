...
const renameProject = async (project:Project,event:Event) => {
  ...
  try { await $fetch(`/api/projects/${project.id}`,{method:'PATCH',body:{name:nextName}}); await refresh(); message.value='Project renamed.' }
  catch { errorMessage.value='Unable to rename this project. Try a different name.'; (event.target as HTMLInputElement).value=project.name }
}
