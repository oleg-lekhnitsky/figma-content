...
interface User { id:string; email:string|null; figma_handle:string|null; role:Role; is_active:boolean; last_login_at:string|null }
...
const email = ref(''); const role = ref<Role>('viewer'); const message = ref('')
const invite = async () => { message.value=''; try { await $fetch('/api/admin/users',{method:'POST',body:{email:email.value,role:role.value}}); email.value=''; await refresh(); message.value='Invitation added.' } catch { message.value='Unable to add this user.' } }
...
<template>
  <div class="admin-shell">
    <header class="toolbar">
      ...
      <nav aria-label="Administration"><NuxtLink to="/admin/users" aria-current="page">Users</NuxtLink><NuxtLink to="/admin/audit-log">Audit log</NuxtLink></nav>
    ...
    <main>
      ...
      <form class="invite" @submit.prevent="invite">
        ...
        <label><span class="sr-only">Email</span><input v-model="email" required type="email" placeholder="Email address"></label>
        <label><span class="sr-only">Role</span><select v-model="role"><option>viewer</option><option>contributor</option><option>editor</option><option>admin</option></select></label>
      ...
      <section class="people" aria-label="Approved users">
        ...
        <article v-for="user in data?.data.users" :key="user.id" class="person">
          <div class="person-name"><strong>{{ user.email ?? user.figma_handle }}</strong><span>{{ user.figma_handle ? `@${user.figma_handle}` : 'Awaiting first Figma login' }}</span><span>Last active {{ user.last_login_at ? new Date(user.last_login_at).toLocaleDateString() : 'never' }}</span></div>
          ...
          <button class="text-action" @click="revoke(user)">Revoke all</button>
        </article>
      </section>
    </main>
  </div>
