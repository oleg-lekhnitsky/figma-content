...
export const matchingAssetIds = async (
  ...
  const uploadedBys = filters.uploadedBys.length ? filters.uploadedBys : filters.uploadedBy ? [filters.uploadedBy] : []
  if (uploadedBys.length) query = query.in('uploaded_by', uploadedBys)
...
export const matchingApprovedAssetIds = (organizationId: string, filters: PublicCollectionFilters) => (
  matchingAssetIds(organizationId, filters, 'approved')
)

const visibleStaticAssetIds = async (collectionId: string, organizationId: string, assetScope: BoardAssetScope) => {
  const { data: membership, error: membershipError } = await useSupabaseAdmin().from('public_collection_assets')
    .select('asset_id').eq('collection_id', collectionId).order('position', { ascending: true, nullsFirst: false }).limit(500)
  if (membershipError) throw databaseError('read collection snapshot', membershipError)
  const ids: string[] = membership.map((item: { asset_id: string }) => item.asset_id)
  if (!ids.length) return []
  let query = useSupabaseAdmin().from('assets').select('id')
    .eq('organization_id', organizationId).in('id', ids)
  query = assetScope === 'all' ? query.neq('status', 'archived') : query.eq('status', 'approved')
  const { data, error } = await query
  if (error) throw databaseError('filter board assets by status', error)
  const visible = new Set(data.map((asset: { id: string }) => asset.id))
  return ids.filter(id => visible.has(id))
}

export const assetIdsForCollection = async (collection: {
  id: string
  ...
  const assetScope = collection.asset_scope ?? 'approved'
  return collection.mode === 'dynamic'
    ? applyDynamicBoardOrder(collection.id, await matchingAssetIds(collection.organization_id, collection.filters, assetScope))
    : visibleStaticAssetIds(collection.id, collection.organization_id, assetScope)
}
...
export const replaceCollectionSnapshot = async (collectionId: string, organizationId: string, filters: PublicCollectionFilters, addedBy?: string, assetScope: BoardAssetScope = 'approved') => {
  const ids: string[] = await matchingAssetIds(organizationId, filters, assetScope)
...
export const addCollectionMatches = async (collectionId: string, organizationId: string, filters: PublicCollectionFilters, addedBy?: string, assetScope: BoardAssetScope = 'approved') => {
  const ids: string[] = await matchingAssetIds(organizationId, filters, assetScope)
  const db = useSupabaseAdmin()
...
export const boardPreviewForCollection = async (collection: {
  ...
  const position = new Map<string, number>(previewIds.map((assetId: string, index: number) => [assetId, index]))
...
export const publicAssetsForCollection = async (
  collection: { id: string; organization_id: string; mode: 'dynamic' | 'static'; filters: PublicCollectionFilters; asset_scope?: BoardAssetScope },
  options: { includeUnapproved?: boolean; includeContributorDetails?: boolean } = {}
) => {
  const includeUnapproved = options.includeUnapproved || collection.asset_scope === 'all'
  const includeContributorDetails = options.includeContributorDetails ?? options.includeUnapproved
  const ids = await assetIdsForCollection({ ...collection, asset_scope: includeUnapproved ? 'all' : 'approved' })
  if (!ids.length) return []
  const selection = includeContributorDetails
  ...
  query = includeUnapproved ? query.neq('status', 'archived') : query.eq('status', 'approved')
  ...
  const position = new Map<string, number>(ids.map((id: string, index: number) => [id, index]))
