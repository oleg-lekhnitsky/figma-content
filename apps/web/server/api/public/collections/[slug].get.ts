...
export default defineEventHandler(async (event) => {
  ...
  const { data, error } = await useSupabaseAdmin().from('public_collections')
    .select('id,organization_id,title,mode,layout,filters,expires_at,created_at,updated_at,organizations(name)')
  ...
  return { data: { collection: { title: collection.title, mode: collection.mode, layout: collection.layout, expiresAt: collection.expires_at, updatedAt: collection.updated_at, organization: collection.organizations }, assets } }
})
