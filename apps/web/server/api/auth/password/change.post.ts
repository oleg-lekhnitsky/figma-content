...
export default defineEventHandler(async (event) => {
  ...
  const { error } = await useSupabaseAdmin().from('allowed_users').update({
  ...
  await useSupabaseAdmin().from('sessions').update({ revoked_at: new Date().toISOString() })
    .eq('allowed_user_id', session.user.id).neq('id', session.id).is('revoked_at', null)
