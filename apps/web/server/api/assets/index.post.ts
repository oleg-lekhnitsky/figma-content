...
export default defineEventHandler(async (event) => {
  ...
  if (!file?.data || !metadataPart?.data) throw appError(400, 'INVALID_UPLOAD', 'Choose an image or video and provide its metadata.')
  const maxBytes = Number(useRuntimeConfig().maxUploadBytes)
  if (file.data.byteLength > maxBytes) throw appError(413, 'FILE_TOO_LARGE', `Choose a file smaller than ${Math.floor(maxBytes / 1_048_576)} MB.`)
  if (!file.type || !isAllowedUploadMime(file.type)) throw appError(415, 'INVALID_MIME_TYPE', 'Upload a PNG, JPG, or MP4 file.')
  ...
  const image = video ? null : sharp(file.data, { failOn: 'error' })
  const info = image ? await image.metadata().catch(() => null) : null
  if (video && (!isValidMp4(file.data) || !parsed.data.width || !parsed.data.height)) throw appError(415, 'INVALID_VIDEO', 'The file contents do not match a valid MP4 video.')
  if (mimeType !== 'video/mp4' && (!info?.width || !info.height || info.format !== expectedSharpFormat(mimeType))) throw appError(415, 'INVALID_IMAGE', 'The file contents do not match a valid PNG or JPG image.')
  ...
  const thumbnailPath = video ? null : r2AssetPath(`${basePath}/thumbnail.webp`)
  const thumbnail2xPath = video ? null : r2AssetPath(`${basePath}/thumbnail@2x.webp`)
  ...
  try {
    await uploadAssetObject(imagePath, file.data, mimeType)
  ...
  const { data: asset, error } = await db.from('assets').insert({
    ...
    width, height, image_format: extension, figma_file_key: metadata.figmaFileKey,
  ...
  if (error) {
    await removeAssetObjects([imagePath, thumbnailPath, thumbnail2xPath].filter((path): path is string => Boolean(path)))
  ...
  await db.from('asset_versions').insert({
    ...
    thumbnail_path: thumbnailPath, thumbnail_2x_path: thumbnail2xPath, mime_type: file.type, file_size: file.data.byteLength,
    width, height, metadata, created_by: session.user.id
  })
