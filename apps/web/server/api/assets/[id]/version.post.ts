...
export default defineEventHandler(async (event) => {
  ...
  const imagePath = `${basePath}/original.${extension}`; const thumbnailPath = `${basePath}/thumbnail.webp`
  ...
  const update = { image_path: imagePath, thumbnail_path: thumbnailPath, mime_type: file.type, file_size: file.data.byteLength, width: info.width, height: info.height, image_format: extension, figma_file_key: metadata.figmaFileKey, figma_node_id: metadata.figmaNodeId, figma_node_name: metadata.figmaNodeName, figma_url: metadata.figmaUrl, version, status: 'draft' }
  const db = useSupabaseAdmin()
  const { error: versionError } = await db.from('asset_versions').insert({ organization_id: session.user.organization_id, asset_id: id, version, image_path: imagePath, thumbnail_path: thumbnailPath, mime_type: file.type, file_size: file.data.byteLength, width: info.width, height: info.height, metadata, created_by: session.user.id })
  if (versionError) { await bucket.remove([imagePath, thumbnailPath]); throw databaseError('create asset version', versionError) }
  ...
  if (error) { await db.from('asset_versions').delete().eq('asset_id', id).eq('version', version); await bucket.remove([imagePath, thumbnailPath]); throw databaseError('activate asset version', error) }
