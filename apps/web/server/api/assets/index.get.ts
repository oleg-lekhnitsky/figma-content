...
export default defineEventHandler(async (event) => {
  ...
  const buildAssetQuery = () => {
    let query = useSupabaseAdmin().from('assets')
      .select('id,title,description,thumbnail_path,thumbnail_2x_path,image_path,mime_type,width,height,status,figma_url,uploaded_by,created_at,updated_at,language,content_type,projects(name),asset_tags(tags(id,name,slug)),allowed_users!assets_uploaded_by_fkey(figma_handle,avatar_url)', { count: 'exact' })
