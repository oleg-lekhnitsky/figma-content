...
export default defineEventHandler(async (event) => {
  ...
  const { data, count, error } = await query.range(from, from + q.pageSize - 1)
  if (error) throw databaseError('list assets', error)
  ...
  const { data: submitters, error: submittersError } = submitterIds.length
